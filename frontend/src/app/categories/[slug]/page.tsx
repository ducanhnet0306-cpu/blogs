...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `Danh mục: ${slug.replace(/-/g, ' ')}` };
}
...
export default async function CategoryPostsPage({ params }: Props) {
  ...
  try {
    const res = await apiFetch<PaginatedResponse<Post>>(
      `/categories/${slug}/posts?per_page=12`,
      { cache: 'no-store' }
    );
    posts = res.data;
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      ...
      <nav className="mb-8 flex flex-wrap items-center gap-1.5 text-sm text-slate-400">
        ...
        <span>/</span>
        <Link href="/categories" className="transition hover:text-blue-600 dark:hover:text-blue-400">
          Danh mục
        </Link>
        <span>/</span>
        <span className="capitalize text-slate-600 dark:text-slate-300">{displayName}</span>
      </nav>

      <h1 className="capitalize text-3xl font-bold text-slate-900 dark:text-white">{displayName}</h1>
